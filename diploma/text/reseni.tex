...
\textbf{\textit{Finalization}} popisuje, za bylo vytváření média ukončeno. Jedná se o binární stav, médium je buď ukončeno (\textit{Closed}, případně \textit{Finalized} nebo otevřeno \textit{Open}). Tento stav je odvoze od počtu AVDP, který je uložený na médiu. Pokud je uloženo pouze jediné (na sektoru 256 nebo 512) médium je otevřené (\textit{Open}) a jeho struktura metadat může být ještě měněna v rámci zápisu. Pokud je uloženo AVDP více (alespoň 2), médium je považované za dokončené (\textit{Finalized}).\\ 
\begin{figure}[] 
    \centering
...
\subsection{Volume Recognition Sequence}
\label{subsec:vrs}
...
\subsubsection{Logical Volume Descriptor}
\label{subsec:lvd}
...
\subsubsection{Partition Descriptor a Space Bitmap Descriptor}
\label{subsec:pd-sbd}
...
V implementaci ve výpočetní technice je toto obvykle realizováno nad sekvencí z nejmenšího datového typu, t.j. nad bytem, který může nabývat hodnot $<0;2^{8}-1>$. Datový typ kontrolního součtu bývá také o velikosti 1~B, ale může být i větší. Pokud hodnota součtu překoná maximální velikost použitého datového typu, výsledek je oříznut a je zachována jeho méně významná část.\\
Příklad výpočtu kontrolního součtu pro 5~B dlouhou sekvenci $S$ a 1~B checksum $C$.
\begin{equation} \label{eq:st0}
    ...
    \end{split}
\end{equation}
Na řádku \ref{eq:st0} je vidět výchozí stav výpočtu. 
...
\begin{equation} \label{eq:st3}
    C = C + S(3) = 165
\end{equation}
Na řádcích \ref{eq:st1}, \ref{eq:st2} a \ref{eq:st3} proběhlo postupné přičítání hodnot sekvence $S$.
\begin{equation} \label{eq:st4}
    ...
    \end{split}
\end{equation}
Na řádku \ref{eq:st4} došlo po přičtení další hodnoty k přetečení povoleného rozsahu. Proto bylo nutné jej oříznout (odečíst) dokud nebyla hodnota nižší než povolená. Poté výpočet pokračuje dál.
\begin{equation} \label{eq:st5}
    C = C + S(5) = 89
\end{equation}
Na řádku \ref{eq:st5} je přičtení poslední hodnoty a výsledek. Kontrolní součet je tedy v tomto případě $C=89$.
\subsection{Cyclic Redundancy Check}
\label{subsec:crc}
\textit{Cyclic Redundancy Check} (CRC) je složitější varianta kontrolního součtu. 
\todo{Přepsat}
...
Struktura nástroje je rozdělena do třech částí. V první části probíhá načtení média a parsování vstupních parametrů. Druhá část detekuje přítomnost UDF, načítá deskriptory souborového systému a strom souborů a detekuje chyby v nich. Třetí část opravuje chyby na základě výsledků z předchozí části.\\
Pokud rozebereme implementaci na menší části, dostaneme se již na úroveň jednotlivých funkcí implementace. Tato struktura je zachycena v obrázku \ref{fig:steps1}.
Závěrem implementace je začlenění výsledného \texttt{udffsck} zpět do balíčku \texttt{udftools}, ze kterého je rozvětven. Tento krok předpokládá funkční a v rámci možností otestovaný nástroj s vytvořenou manuálovou stránkou a testovacím nástrojem pro automatické zachování funkčnosti po začlenění.\\
\todo{opravit obrázek na novou verzi}
\begin{figure}[ht] 
    \centering
    \includegraphics[scale=0.4]{obrazky/steps1.eps}
    \caption{Algoritmus činnosti nástroje}
    \label{fig:steps1}
\end{figure}
