...
\subsection{Načtení a kontrola AVDP}
...
\subsection{Načtení a kontrola VDS}
...
\subsection{Načtení a kontrola LVID}
...
Poslední položkou, která je důležitá pro prácí se svazkem je \textit{Integrity Type}. Ten může být buď otevřený nebo uzavřený. Pokud je uzavřený, tak by měl být svazek v konzistentním stavu. Pakliže je otevřený, je možné že probíhá zápisová operace, nebo že byl svazek během ní nesprávně odebrán. Tento indikátor pomáhá vyvolat požadavek na kontrolu média v operačním systému a pro kontrolu to je důležitý ukazatel upozorňující na pravděpodbnou nesrovnalost mezi deklarovaným volným místem a skutečností. 

\subsection{Kontrola a obnova stromu souborů}
\label{sec:filetree}
\todo{Popsat Unique ID}
...
\subsection{Mapa chyb}
\label{sec:mapa-chyb}
Účelem takzvané mapy chyb (neboli struktury \texttt{\detokenize{vds_sequence_t}}) je zapouzdření chyb, které jsou deteknovány napříč médiem v deskriptorech, vyjma chyb v deskriptorech dat. Její struktura je v tabulce \ref{tab:err-seq}.\\
...
\begin{table}
    \begin{tabular}{ | l | l | l | }
        ...
        \texttt{E\_WRONGDESC}& \texttt{0b00001000} & Nenalezen očekávaný deskrioptor (použito pouze pro AVDP)\\ \hline
...
Oprava AVDP je možná pouze na uzavřených médiích. Důvodem je požadavek opravného algoritmu na přítomnost správného deskriptoru, který bude použit jako vzor pro opravu. Otevřená (neukončená) média mají pouze jeden AVDP, proto v případě jeho poškození není oprava snadno možná.\\
...
\subsection{Korekce VDS}
\label{sec:oprava-vds}
Oprava \textit{Volume Descriptor Sequence} je podovná jako AVDP. Předpokladem úspěšné opravy je opět existence alespoň jednoho správného zdrojového deskriptoru od každého typu, který ve VDS je. Pokud je toto splněno, je oprava možná.\\
...
\begin{enumerate}
    ...
    \item Krok 2 se opakuje dokud není nalezen \textit{Terminating Descriptor}. Poté je oprava ukončena.
\end{enumerate}

\subsection{Korekce LVID}
